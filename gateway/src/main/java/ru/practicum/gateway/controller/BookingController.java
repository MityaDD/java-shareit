...
public class BookingController {
    ...
    @PostMapping
    ResponseEntity<Object> addReservation(@RequestHeader(HEADER) @Min(1) Long userId,
    ...
    ResponseEntity<Object> getBookingById(@PathVariable @Min(1) Long bookingId,
                                          @RequestHeader(HEADER) @Min(1) Long userId) {
        return client.getBookingById(userId, bookingId);
    }

    @GetMapping
    ResponseEntity<Object> getAllReservationsByUserId(@RequestHeader(HEADER) @Min(1) Long userId,
    ...
    @GetMapping("/owner")
    ResponseEntity<Object> getAllReservationsByOwnerId(@RequestHeader(HEADER) @Min(1) Long userId,
                                                       @RequestParam(value = "state", defaultValue = "ALL") String stateString,
                                                       @RequestParam(defaultValue = "0") @PositiveOrZero int from,
                                                       @RequestParam(defaultValue = "10") @Positive int size) {
        State state = State.from(stateString)
